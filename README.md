...
サブフォルダ20個・各60ファイル(合計1,200件)の`.md`ファイルを一時的に生成し、`Measure-Command`でファイル探索から見出し検索までの所要時間を計測しました(Pester不要、`pwsh`標準機能のみで実施。フィクスチャはリポジトリには含めていません)。
